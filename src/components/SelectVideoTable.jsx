...
const SelectVideoTable = (props) => {
    const { loading, videos, selectedVideo, sendVideo } = props;

     const GlobalResFilter = ({ filter, setFilter }) => {
       const inputRef = useRef(null);

       useEffect(() => {inputRef.current.focus();}, [filter]);
       return (
     ...
     const headersColumn = (name, value2) => {
       return {
         ...
         id: name,
         Cell: (props) => (
           <div style={{ textAlign: "left" }}>{props.value}</div>
         ),
       };
     };
               ...
               <div>
                 <i
                   className={`btn add_button me-3 mt-2 ${
                     selectedVideo.find(
                       (selectedVideo) =>
                         selectedVideo._id === props.cell.row.original._id
                     )
                       ? "btn-success"
                       : "btn-primary"
                   }`}
                   aria-hidden="true"
                   onClick={(event) => {
                     sendVideo(event, props.cell.row.original);
                   }}
                 >
                   {selectedVideo.find(
                     (selectedVideo) =>
...
export default SelectVideoTable;